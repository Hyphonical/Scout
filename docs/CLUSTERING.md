...
Scout uses advanced density-based clustering to automatically group your media (images and videos) by visual similarity. Unlike traditional methods like K-Means, Scout doesn't require you to specify the number of clusters beforehand and can identify "noise" (outliers) that doesn't belong anywhere.
...
In Scout, UMAP reduces the **1024D** embeddings to **512D** (or lower) to speed up HDBSCAN while preserving the local and global structure of your media library.
...
- **Variable Density**: It finds clusters that have different densities, which is common in media collections (e.g., hundreds of "beach" photos vs. five "sunset" videos).
...
Scout finds the "archetype" for each cluster—the file that best represents the group. It calculates the **Centroid** (mean vector) in the original 1024D space:
$$\mu = \frac{1}{N} \sum_{i=1}^{N} E_i$$
The file whose embedding has the highest **Cosine Similarity** to $\mu$ is selected as the representative:
$$\text{Representative} = \arg\max_{i} \left( \frac{E_i \cdot \mu}{\|E_i\| \|\mu\|} \right)$$

### Cohesion Score
To tell you how "tight" a cluster is, Scout calculates a **Cohesion Score** (0-100%). This is the average pairwise similarity between all files in the cluster:
$$\text{Cohesion} = \frac{2}{N(N-1)} \sum_{i < j} \text{sim}(E_i, E_j)$$
A cluster of identical content will have ~100% cohesion, while a loose group of "nature" photos might have 70-80%.

---

## Summary of Parameters

- `min-cluster-size`: The minimum number of files needed to form a group (default: 5).
- `min-samples`: Adjusts how conservative the clustering is. Higher values result in more noise and fewer, more "core" clusters.
- `use-umap`: Enables UMAP dimension reduction. Recommended for collections larger than 1,000 files.
- `export`: Export cluster results as JSON for further processing and automation.
