...
pub fn run(
	...
	let video_supported = processing::video::is_available();
	if !video_supported {
	...
	for file in scan_result.to_process {
		...
		match result {
			...
			Err(e) => {
				ui::error(&format!("Failed {}: {}", file.filename, e));
				errors += 1;
			}
		}
	}
	...
	println!();
	ui::success(&format!("Processed {} files in {:.1}s", processed, duration));
	...
	if skipped_videos > 0 {
		ui::info(&format!("{} videos skipped (FFmpeg not available)", skipped_videos));
	}
...
fn process_image(
	...
	let sidecar = storage::ImageSidecar::new(
		file.filename.clone(),
		file.hash.clone(),
		embedding,
	);
...
fn process_video(
	...
	let sidecar = storage::VideoSidecar::new(
		file.filename.clone(),
		file.hash.clone(),
		encoded_frames,
	);
